"""
Subscraper: koncerty (klasická/vážná hudba) z prague.eu

Stejná struktura jako prague_vystavy.py — URL nese datum i stránkování:
  https://prague.eu/cs/akce-kategorie/klasicka-hudba/?pg=1&start_date=DD-MM-YYYY&end_date=DD-MM-YYYY

Server-side HTML, žádný JS → requests + BeautifulSoup. Pozn.: prague.eu i pro
koncerty vrací pořád stejnou stránku (pg=2 == pg=1), takže reálné stránkování
nefunguje a anti-loop přes data-poid se zastaví hned po první straně — ale
kdyby zdroj někdy začal stránkovat, tahle smyčka to projede automaticky.

Čas a cena: prague.eu je pro koncerty nedodává strojově čitelně (žádný Event
schema, v textu nekonzistentní), proto je scraper nechává null. Když jsou
v popisu explicitně, dolije je až AI krok (cowork_prompt_koncerty.md).
"""
...
BASE = "https://prague.eu/cs/akce-kategorie/klasicka-hudba/"
...
MAX_TERMINY = 10  # strop na počet termínů — noční turistické cykly (Vivaldi/Mozart každý
#                   večer) generují stovky termínů, což nafoukne RAW soubor a popup je stejně
#                   k ničemu. datumOd/datumDo drží plný rozsah, takže info „běží celé léto" nemizí.
...
def _terminy(tile, casova_mapa):
    ...
    a ořízne na MAX_TERMINY (noční jarmarky mají klidně 178 termínů).
    """
    ...
    while True:
        ...
        for tile in tiles:
            ...
            p = polozka(
                ...
                # u koncertu je afterHeading přímo místo (síň/kostel/klub)
        ...
        print(f"  [prague.eu/koncerty_klasika] strana {pg}: {len(tiles)} akcí")
        pg += 1
        time.sleep(PAUZA)
